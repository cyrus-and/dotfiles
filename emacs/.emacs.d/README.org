...
Define a local variable (default ~t~) which determines whether the ~before-save-hook~ should trim trailing whitespaces or not:

#+BEGIN_SRC emacs-lisp
(defvar my/delete-trailing-whitespace-on-save t
  "If t calls `delete-trailing-whitespace' on `before-save-hook'.")

(make-variable-buffer-local 'my/delete-trailing-whitespace-on-save)

(defun my/before-save-hook ()
  (when my/delete-trailing-whitespace-on-save
    (delete-trailing-whitespace)))

(add-hook 'before-save-hook 'my/before-save-hook)
#+END_SRC

Add a function to toggle the above variable:

#+BEGIN_SRC emacs-lisp
(defun my/toggle-delete-trailing-whitespace-on-save ()
  "Toggle `my/delete-trailing-whitespace-on-save' interactively."
  (interactive)
  (let ((enabled (not my/delete-trailing-whitespace-on-save)))
    (setq my/delete-trailing-whitespace-on-save enabled)
    (when enabled
      (set-buffer-modified-p t))
    (message (format "delete-trailing-whitespace-on-save: %s" enabled))))
#+END_SRC

Add convenience shortcuts:

#+BEGIN_SRC emacs-lisp
(global-set-key (kbd "C-c d") 'my/toggle-delete-trailing-whitespace-on-save)
#+END_SRC
