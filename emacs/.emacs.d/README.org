...
Custom keybindings (list with ~C-c ?~):
...
More confortable mouse scrolling and yanking:
...
Credentials are fetched from [[https://www.passwordstore.org/][~pass~]] (a graphical pinentry for GPG must be available).
...
Do not anny with trivial confirmations:
...
Note that the check must be perfomed /live/ becouse it should work even if the Emacs server is used.
