...
Note that setting ~scroll-conservatively~ to a large number yileds the same result but it also affects the point position after a /jump/, e.g., ~next-error~, ~iserach-forward~, etc.
...
Avoid that a reconnection pop ups the ERC buffer:
...
Disable automatic point recentering so the prompt stays still (unless the window is resized):
