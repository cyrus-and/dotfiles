...
At startup Emacs loads [[file:init.el][init.el]] which in turn tangles this file producing the main configuration file (~my-init.el~) and others, but ony if not yet present or obsolete.
...
Silently auto-instert skeletons in new files and mark them as modified:
